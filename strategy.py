...
def get_and_transform_mt5_data(symbol, timeframe, number_of_candles, pip_size):
    ...
    # print("type of pip size", type(pip_size))
    # print("type of df[high]", type(df_data['high'][0]))
    df_data['trade_high'] = df_data['high'] + float(pip_size)
    # Calculate trade_low (low price - 1 pip)
    df_data['trade_low'] = df_data['low'] - float(pip_size)
    # Calculate the number of pips between trade_high and trade_low
    df_data['pip_distance'] = (df_data['trade_high'] - df_data['trade_low'])/float(pip_size)
...
def create_new_order(decision_outcome, candle_dataframe, pip_size, symbol):
    ...
    if decision_outcome == "DoNothing":
        ...
        num_pips = first_row["pip_distance"] * 2 * float(pip_size) # Convert pip_distance back into pips
        take_profit = buy_stop + num_pips
        # Add in an order comment
        comment = "Green Order"
        # Send order to place_order function in mt5_interface.py
        print('Buy stop', symbol, '0.1', buy_stop, stop_loss, take_profit, comment)
        ...
        comment = "Red Order"
        # Send order to place_order function in mt5_interface.py
        print('Sell stop', symbol, '0.1', buy_stop, stop_loss, take_profit, comment)
